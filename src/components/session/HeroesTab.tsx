...
import { CLASS_BY_ID, HERO_BY_ID, resolveItemName, withClass, uid } from './sessionHelpers'
import { ChipToggle, TextInput, SubHeading } from './ui'

const MAX_HEROES = 4

// ── Item-Picker-Modal (Marktkarten + Helden-Relikte + eigener Gegenstand) ─────
...
function ItemPickerModal({
  ...
  return (
    <ModalOverlay
      onClose={onClose}
      ariaLabel="Gegenstand hinzufügen"
    ...
    >
      <div className="p-4 border-b border-dungeon-700 flex items-center justify-between gap-3">
        <h3 className="font-display text-lg text-gold-300 font-bold">Gegenstand hinzufügen</h3>
...
function HeroSetupCard({
  ...
  return (
    <div className="card space-y-4">
      {itemPicker && (
        <ItemPickerModal ownedExpansionIds={ownedExpansionIds} onPick={addItem} onClose={() => setItemPicker(false)} />
      )}
