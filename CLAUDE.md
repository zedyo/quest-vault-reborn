...
## Aktuelle Version: 1.6.8 (2026-07-18)
...
| v1.6.8 | ✅ Abgeschlossen | **Errata-Overwrite-Audit + Kartentext-Treue.** User-Frage: Wurden Original-Karten­texte durch Errata-Fassungen überschrieben? **Antwort: Nein.** Systematischer Bild-gegen-Daten-Abgleich aller **164 Karten mit Errata-Bezug** (Helden, Items/Relikte, Overlord, Plotdecks, Monster, Gerüchte — je Karte das deutsche Original-Kartenbild `public/cards/de/**` mit `heroAbility`/`heroicFeat`/`rulesDe`/Monster-Fähigkeiten verglichen) via Multi-Agent-Workflow (Vergleich + adversariale Gegenprüfung, Symbol↔Wort-Normalisierung). Ergebnis: **kein einziger Errata-Overwrite** — `dataMatchesErrata` durchgehend na/no; die CRRG-Errata bleibt korrekt additiv (nur ErrataBox). Wo die deutsche Karte die Errata-Fassung bereits gedruckt hat (z. B. Schattendrache/Schatten), ist das der Kartentext selbst, keine Überschreibung. Klassen (kein DE-Skill-Kartenbild im Repo) zusätzlich textstrukturell geprüft: 14/16 Textkorrektur-Errata belegen, dass der Haupttext die **Vor-Errata-Originalfassung** hält (z. B. Berserker/Gegenangriff, Ritter/Absichern, Waldläufer „vor UND nach"). **Nebenbefund (behoben):** 18 kleine, errata-UNabhängige Abschreibfehler zur Karte korrigiert (Kartentext = priorisierte Wahrheit) — 3 Overlord-Fallen „Wahrnehmung"→**„Gespür"** (kanonischer Attributname), Unterweltdrache+Dunkler Priester „Schreckenerregend/wendet -1 an"→**„Grauenerregend/verringert um 1"**, Plotkarte Bezaubern Attribut `[Geistesgegenwart]`→**`[Willenskraft]`** (Karte + EN „Willpower"), Schattendrache/Feuerodem überzähliges „(inklusive Zielfeld)" entfernt (Halbdrachenkrieger behält es — dessen Karte druckt es), + 5 Helden-Heldentaten (Lindel/Raythen/Roganna/Okaluk/Stahlhorn), Untoter Schädel, Harpyie/Pulk, Höllenkoloss/Sturmangriff, Windarmbänder/„sich", Zeichen des letzten Zenits/„sofort", Beastman/Kommando „darf"→„kann". Alle Fixes vor der Änderung selbst am Kartenbild verifiziert (u. a. shared-ability-Fallen: hybrid-sentinel, dark-priest, steelhorns). **Bewusst NICHT geändert:** deliberate Vereinheitlichung „(nach Verrechnung der Verteidigung)" (Cave-Spider/Zombie-Karten drucken „(nach Abzug der Schildsymbole)"; dokumentierte Projektentscheidung, `de-karten/monster.md`). Docs synchron (`de-karten/{monster,helden,items}.md` + `monsters.md`). Offen: Der Abgleich deckte nur Karten MIT Errata ab; ein voller Karten-Treue-Sweep aller ~500 Karten (nicht-Errata) ist optionaler Folge-Auftrag — Stichproben (Stahlhorn, Halbdrachenkrieger, Dunkler Priester) zeigten vereinzelt gleiche Klein-Drifts auf Nicht-Errata-Karten. 122 Tests + Build grün. |
