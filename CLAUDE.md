...
- **Offizieller Kartenstand übernommen (v1.2.5, User-Freigabe):** Namen +
  Heldenfähigkeit + Heldentat der 54 gescannten Helden kommen jetzt wortgetreu von den
  deutschen Karten – via `src/data/heroOfficialDe.ts` (Generator
  `scripts/build_hero_official_de.py`), als **Override** über die Basis-Literale in
  `heroes.ts` gelegt (`BASE_HEROES.map(... ⇒ {...h, ...official})`). Die **alten
  Community-Werte bleiben als Basis-Literale erhalten** (nichts gelöscht). 16
  Namensabweichungen damit am Kartenstand (z. B. „Eine Faust"→„Einfaust").
  `[…]`-Symbolplatzhalter → ❤/⚡/↻ bzw. Klartext.
