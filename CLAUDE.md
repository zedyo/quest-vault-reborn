...
## Aktuelle Version: 1.8.1 (2026-07-31)
...
> **Versionssprung 1.6.13 → 1.8.0:** Auf Wunsch des Users trägt das Session-Tracker-Redesign
> die Nummer **1.8.0**; **1.7.0 wurde übersprungen**. Die zuvor unter 1.7.0/1.8.0 geplanten
> Vorhaben (Monster-Tracker, Helden-Spieleransicht) sind entsprechend auf **1.9.0/1.10.0**
> weitergerückt — es gibt also keine zweite v1.8.0-Zeile.
...
| v1.6.13 | ✅ Abgeschlossen | **Größen-Pass Icon-Set (User-Wunsch):** Stat-Symbole (Bewegung/Leben/Ausdauer/Verteidigung/Angriffsart) größer — Monster-StatBlock `iconSize` 14/16→**18/21** (compact/Lightbox), Helden-Lightbox 18→**22**, Helden-Grid 12→**16**, Leutnants/Agenten-PerPlayerTable 12→**16** + Angriffsart 15→**19**, MOTD-Werte-Tabelle 15→**19**. Attribut-Embleme: Helden-Chips 12/16→**15/20** (compact/voll), Leutnants-Chips 13→**16**. Archetyp-Scheiben im Helden-/Klassen-Filter 20→**27** (`-my-1.5` — Buttonhöhe bleibt 34 px, per Playwright gemessen). Nur Größenwerte, keine Logik-/Datenänderung. 138 Tests + Build grün; beide Themes per Playwright verifiziert (0 Ladefehler, Buttonhöhen unverändert). |
| v1.8.0 | ✅ Abgeschlossen | **Session-Tracker Redesign — vom Formular zum Kampagnen-Cockpit** (Design-Handoff „Session-Tracker Redesign", 16 Screens, 1:1 umgesetzt). **Neue Informationsarchitektur:** `/session` = Kampagnen-Auswahl (laufend + Archiv), `/session/:id` = Shell mit 74-px-Kopfzeile + **46-px-Abschnittsleiste** (Überblick · Helden · Overlord · Verlauf; „Einrichtung" dezent rechts, kein Tab mehr), `/session/:id/abschluss` = geführter 5-Schritte-Flow. Mobil werden die vier Abschnitte zur **64-px-Fußleiste**. **Vier gelöste Befunde:** (1) Setup ist nicht mehr der erste Tab; (2) die Kachelreihe aus sechs gleich großen Werten ist einer Hierarchie gewichen (Partei-Gold 40 px, offene XP je Held als Zeile, Bedrohung/Schicksal als Stepper bei ihrem Besitzer, „Anzahl Helden" entfällt); (3) **Regeltext immer im Klartext** — der Scan ist nur noch Miniatur mit Lightbox; (4) der Heldenbogen zeigt **ausschließlich besessene** Fähigkeiten, kaufbare erscheinen nur in Flow-Schritt 3. **Zwei Designs als Struktur:** Heldenbereiche rendern in `data-theme="heldentum"`, Overlord-Bereiche in `data-theme="overlord"` — unabhängig vom globalen Schalter (`ThemeScope`). **Neue geteilte Bausteine:** `session/ui/CardTile.tsx` (die EINE Karten-Kachel für Fähigkeit/Gegenstand/Overlord-/Plot-/Gerücht-/Marktkarte, `rulesText` ist Pflichtfeld), `HeroChipRow` (ersetzt ALLE `<select>`-Zuweisungen; mobil vier 44-px-Flächen), `CardThumb` (Miniatur + Lightbox, entfällt < 480 px), `Timeline`, `NextScenarioCard`, `controls.tsx` (Typo-Skala Eyebrow/Micro/Meta/Rules/Head + Btn/Badge/Stepper/Switch/Progress/Segmented/Select/TextField/NumberField/Band/Panel/NoteBox/ThemeScope/CheckBox). **Abschluss-Flow** (Ergebnis → Belohnungen → Erfahrung → Markt → Overlord) arbeitet auf einem **Entwurf** (eigener localStorage-Key `qvr-scenario-draft`, Muster `builderDraft.ts`) mit Fußleiste, laufender Bilanz und „Später fortsetzen"; erst Schritt 5 schreibt — in EINEM `persist` (Szenario + Bedrohungsstand + Schicksal→0 + Kartenzustände). Gerechnet wird gegen den **Stand VOR dem Szenario** (`deriveLiveState` ohne den Entwurf) → XP-Budgets und Gold-Rechnung stimmen auch beim Bearbeiten. **Markt** mit zwei Modi (X Karten aufdecken — zufällig aus dem Akt-Stapel, nur Sammlung, ohne besessene Karten — bzw. manuell suchen über Name UND Regeltext) plus dem **Zwischenspiel-Sonderfall** (voller Akt-I-Stapel vs. X Akt-II-Karten). **Gerüchte & Zusatzabenteuer:** neuer reiner Parser `src/utils/rumorTiming.ts` liest Zeitfenster (6), Akt-Beschränkung, Intermezzo-Sperre, Ausschlüsse, Verfall + Overlord-Belohnung **aus `Rumor.textDe`** (kein handgepflegtes Mapping über 60 Karten); `rumorPlayableNow` setzt „höchstens 1 Gerücht pro Kampagnenphase" durch; nicht spielbare Karten bleiben **sichtbar mit Sperrgrund**. Gerüchte liegen in einer **geteilten Ansicht** (`/verlauf/karten`), nicht mehr als Overlord-Besitz; Zusatzabenteuerkarten sind **kein Teil des Gerüchtedecks**. Gerüchteabenteuer + Zusatzabenteuer stehen im Verlauf und in Flow-Schritt 1 **gleichberechtigt** neben dem Kampagnenbogen; die **Vorschlagslogik** (`scenarioSuggest.ts`) schlägt nur vor, wo der Bogen eindeutig ist — sonst kein Vorschlag (**niemals raten**), inkl. „wer wählt" aus dem Ausgang des letzten Szenarios. **Datenmodell additiv + Migration v2→v3** (`PERSIST_VERSION` 3): `archived`/`note`/`epic`/`rumors`/`advancedQuests`, `TrackedHero.startingXp` (epische Variante, zählt in `xpEarned`) + `startingFateTokens` (freier Startwert, KEINE Regelvorgabe der Variante), `PlayedScenario.playedAt`/`threatAfter`/`fateAfter`/`rumorPlayedId`/`market`; `TrackedOverlord.activeRumorIds` ist `@deprecated` und wird EINMALIG nach `rumors` übertragen (idempotent). `deriveLiveState` bekam genau **drei** Ergänzungen (Start-XP, `heroXpOpenTotal`, `rumorPlayedInCurrentPhase`) — die Anrechnung bleibt ausschließlich dort. **Keine Emoji** mehr im Tracker: 22 neue Glyphen in `QvIcons.tsx` (plus/close/check/edit/trash/save/download/upload/print/filter/sort/info/warning/external/more/gold/xp/chevron-×4). Neu: Kampagne **archivieren** statt löschen, freie **Spielstand-Notiz**, v1.9-Platzhalter „Monster-Tracker". **Härtung (aus dem Sicherheits-Review):** `heroXp`-Schlüsselzahl gedeckelt + reservierte Schlüssel (`__proto__`/`constructor`/`toString` …) als Helden-Schlüssel und `localId` verworfen, `Math.max(...xp)` durch `reduce` ersetzt (kein Render-Crash durch präparierte Importe); ein leerer Kampagnenname löscht den Stand beim Hydrieren nicht mehr (Ersatzname, strikte Prüfung bleibt beim Import); Entwurfs-Key **je Kampagne** (`qvr-scenario-draft:<sessionId>`); „Neu ziehen" behält gespielte/verfallene Gerüchte. **Härtung Runde 2:** die Liste der verbotenen Schlüssel wird aus `Object.getOwnPropertyNames(Object.prototype)` ABGELEITET statt von Hand gepflegt (vorher fehlten u. a. `isPrototypeOf`/`propertyIsEnumerable` → `heroXp[localId] ?? 0` hätte eine geerbte Funktion geliefert und `xpEarned` zu einem String gemacht); neue `safeId`-Prüfung (`/^[A-Za-z0-9_-]{1,100}$/`) für Session-/Szenario-/Helden-IDs, damit importierte IDs weder als Rohtext in eine Route (`?edit=`, `/helden/:id`) noch als Objektschlüssel geraten — die Ersatz-ID ist **deterministisch** (djb2), damit die inneren Verweise heil bleiben (`ItemSold.refId` ↔ `ItemRef.refId`, `toHeroLocalId`/`heroLocalId`/`heroXp`-Schlüssel ↔ `TrackedHero.localId`, `playedInScenarioId` ↔ `PlayedScenario.id`) und ein zweiter Durchlauf nichts mehr verschiebt (idempotent, per Test belegt); beim Löschen einer Kampagne wird ihr Flow-Entwurf mit entfernt (keine Waisen im localStorage); der Entwurf wird einmal je Kampagne gelesen (`useMemo`) statt bei jedem Render; **Marker-Schutz beim Nachbearbeiten** — Bedrohung/Schicksal schreibt nur der jüngste Eintrag fort, eine Korrektur an Szenario 2 von 10 setzt die aktuellen Zähler nicht mehr still zurück. **Regel-Korrekturen aus dem Daten-Review (alle gegen die CRRG-Wissensbasis belegt):** Akt-Beschränkung eines Gerüchts wird NUR aus dem Auslösersatz gelesen (sonst las „Zweifelhafte Schätze" ihren Preisabsatz „in Akt II" als Sperre, obwohl die Karte kein Aktsymbol trägt); `intermezzoSelectable` rät nicht mehr, sondern bejaht nur den ableitbaren Fall (regulärer Akt-I-Pool gespielt) — vorher waren die zwölf Gerüchteabenteuer mit Intermezzo-Klausel faktisch den ganzen Akt I gesperrt; die Verfallswarnung nennt keine erfundene „Belohnung von der Kartenrückseite" mehr (Akt-I-Gerüchte haben keine Belohnungsrückseite) und die Finale-Regel erscheint nur bei den 12 Zusatzabenteuern, die sie tragen; XP-Vorbelegung folgt der Regel „jeder Spieler erhält 1 EP, egal wer gewonnen hat"; Standardwerte auf die belegten Zahlen (5 Marktkarten aufdecken, 3 Gerüchte ziehen); Verkaufserlös = halber Wert auf 25 abgerundet, Startausrüstung pauschal 25, **Relikte nicht verkäuflich**; Begleiter-Verteidigung als Würfelfarbe statt als Zahl; der Fortschrittsbalken nennt den Nenner ehrlich („N von M Titeln des Kampagnenbogens") und zählt im Zähler auch nur GESPIELTE Titel des Bogens (`playedCampaignIds`), nicht jeden Protokolleintrag. **Zweite Review-Runde:** der Verfallshinweis in Flow-Schritt 1 nennt jetzt (wie schon die Kartenansicht) nur die Karten, die die Klausel wirklich tragen — Gerüchteabenteuer mit `expiresAtActTransition`, Zusatzabenteuer mit `overlordRewardIfUnplayed` (12 von 16) — und spricht von der „auf der Karte genannten Belohnung" statt von einer Kartenrückseite; die auf einer Gerüchtekarte GENANNTE Zusatzabenteuerkarte wird über `resolveAdvancedQuestId` **normalisiert** aufgelöst (Apostroph-Varianten: „Devis Blutturm" auf der Gerüchtekarte ↔ „Devis' Blutturm" auf der Zusatzabenteuerkarte — beide Transkriptionen sind kartengenau und bleiben unangetastet; vorher kam diese eine Karte nie ins Spiel); die Aussage „Gerüchte sind beiden Seiten bekannt" ist als Werkzeug-Aussage umformuliert (keine Regelgrundlage). **Dritte Review-Runde:** die **epische Variante** behauptete Start-Schicksalsmarker — belegt sind aber nur Start-EP + Start-Gold (Fortgeschrittene 3 EP/150 G, Experten 6 EP/250 G, Overlord 4 bzw. 8 EP); Schicksalsmarker entstehen erst im Spiel, wenn der Overlord Bedrohung ausgibt. Der Setup-Text nennt jetzt die belegten Zahlen, und das Schicksal-Feld ist ausdrücklich als **freier Startwert ohne Regelgrundlage** (Standard 0) gekennzeichnet — Feld und Migration bleiben unverändert. Ebenfalls präzisiert: Akt-II-Marktkarten „solange noch kein Akt-II-Abenteuer beendet ist" (Regelwortlaut) + Mini-Kampagnen-Sonderfall, Zwischenspiel-Band im Verlauf neutral „Übergang zu Akt II" statt „Markt Akt I + II", `intermezzoSelectable`-Kommentar korrigiert (die Bedingung steht für 2 der 9 Kampagnen sehr wohl in den Regelklärungen, nur nicht maschinenlesbar), Kampagnenname im Doc-Kommentar „Der Schatten der Rune"→**Die Schattenrune**. **Datenkorrektur:** 12 der 16 `ADVANCED_QUESTS.nameDe` waren Community-Übersetzungen und wichen vom deutschen Kartentitel ab (derselbe Datensatz liegt unter gleicher ID in `rumors.ts`) → kartengenau korrigiert (z. B. „An der Schmiede"→**Die Waffenschmiede**, „Wo das Herz wohnt"→**Herzensangelegenheit**), Doku (`docs/game-data/campaigns.md`) + Datenintegritäts-Test dazu. Tests: `rumorTiming` (27, inkl. „jede genannte Zusatzabenteuerkarte ist auflösbar") + `scenarioSuggest` (12) + Verkaufs-/Titel-Regression + erweiterte Derive-/Import-Tests (ID-Form, geerbte Object-Member als XP-Schlüssel) → **194 grün**; Build grün; in beiden Designs + mobil (390 px) per Playwright im echten Build verifiziert (kompletter Flow-Durchlauf, 0 Konsolenfehler, Tippziele ≥ 44 px). Wissensbasis `wiki/entities/session-tracker.md`. |
| v1.8.1 | ✅ Abgeschlossen | **Dependency-Sicherheitspass — alle Dependabot-Meldungen behoben (`npm audit` = 0).** 12 offene Advisories (1 kritisch/4 hoch/6 moderat/1 niedrig) + 3 offene Dependabot-PRs (#97/#98/#99, die jeweils nur Teilmengen abdeckten) durch EINEN vollständigen Upgrade-Pass ersetzt. **Zwingende Kette bei react-router:** die zwei Advisories `GHSA-wrjc-x8rr-h8h6` (`6.0.0–7.17.0`) und `GHSA-qwww-vcr4-c8h2` (`>=7.12.0 <8.3.0`) überlappen so, dass **keine** v6/v7-Version sauber ist → `react-router@8.3.0`, das seinerseits `react >= 19.2.7` und `node >= 22.22.0` verlangt. **`react-router-dom` wird ab v8 nicht mehr veröffentlicht** (war in v7 nur noch ein Re-Export-Shim) → alle 24 Importe auf `from 'react-router'` umgestellt; die genutzte API (BrowserRouter/Routes/Route/Link/NavLink/Outlet/useNavigate/useParams/useSearchParams/useLocation/useOutletContext) ist identisch, **kein Code-Umbau**. **React 18→19** war deshalb Pflicht — Migrationsfläche minimal (kein forwardRef/React.FC/defaultProps, `createRoot` schon da, alle `useRef` mit Initialwert); **einzige** Anpassung: `import { type JSX }` in QuestEditorPage, da @types/react 19 den globalen JSX-Namespace entfernt. **brace-expansion-Falle:** erst 5.0.8+ ist sauber, aber dessen CJS-Export ist kein aufrufbarer Default mehr (`{EXPANSION_MAX, expand}`) → ein pauschales Override bricht `minimatch@3/5`; stattdessen zielgenau `overrides: { filelist: { minimatch: '^10.2.6' } }` (die einzige Linie auf `brace-expansion@^5`; `minimatch.match()`-Kompatibilität zu `filelist` vorab in Node verifiziert). Weiter: vite 5→8, vitest 2→4, @vitejs/plugin-react 4→6, vite-plugin-pwa 0.20→1.3, postcss/autoprefixer/tailwind/typescript auf aktuelle Patches, **CI+Deploy-Node 20→22** (20 ist seit April 2026 EOL). Vite 8 verlangt `import pkg from './package.json' with { type: 'json' }`. **Bewusst NICHT mitgezogen** (kein Advisory, unnötiges Risiko): tailwind 4, zustand 5, typescript 7. Keine Spieldaten-, Store-, Persist- oder UI-Änderung. Verifikation: 195 Tests + Build grün, `npm audit` = 0, und der **echte Produktions-Build** per Playwright über 16 Routen × beide Designs + SPA-Navigation ohne Reload + `useSearchParams` + mobil 390 px, 0 JS-Fehler. Wissensbasis `wiki/concepts/dependency-security-pass.md`. |
| v1.8.1a (Nachtrag, keine App-Änderung) | ✅ Abgeschlossen | **GitHub-Pages-Deploy repariert: zwei konkurrierende Deployment-Wege entkoppelt.** Nach dem v1.8.1-Merge war `Deploy to GitHub Pages` grün, GitHubs **eingebauter** `pages-build-deployment` aber **rot** (Run #151). Ursache war NICHT das Dependency-Upgrade, sondern ein Alt-Konflikt in `deploy.yml`: der Schritt `peaceiris/actions-gh-pages@v4` pushte `dist/` in den `gh-pages`-Branch → dieser Push löste GitHubs eingebauten Workflow aus, der PARALLEL zum eigenen `deploy`-Job ein **zweites** Pages-Deployment anlegen wollte. GitHub serialisiert Pages-Deployments, der Verlierer bricht mit HTTP 400 ab (`Deployment request failed for 476a628e due to in progress deployment. Please cancel 82743ce1 first`). Weil es ein **Wettlauf** ist, schlug es nur sporadisch fehl (davor zuletzt Run #123 am 2026-07-05, dazwischen 27 grüne Läufe) — die Seite war nie kaputt, es blieb ein rotes X. **Fix:** `peaceiris`-Schritt entfernt (es bleibt der Artefakt-Weg `configure-pages` → `upload-pages-artifact` → `deploy-pages`; ohne `gh-pages`-Push wird der eingebaute Workflow gar nicht mehr ausgelöst), `contents: write` → **`contents: read`** (der Branch-Push war der einzige Grund für Schreibrechte), `cancel-in-progress` → **`false`** (dokumentierte Empfehlung für Deployment-Workflows). Beleg für die Richtung: `actions/deploy-pages@v4` lief erfolgreich durch — das geht nur bei Pages-Quelle „GitHub Actions“. Der `gh-pages`-Branch bleibt als Rückfallebene liegen, wird aber nicht mehr beschrieben. **Neue Dauerregel in „CI als Sicherheitsnetz“: nach jedem Merge BEIDE Pages-Workflows prüfen.** Keine Code-, Daten- oder Abhängigkeitsänderung — nur `.github/workflows/deploy.yml`. **Bewusst KEIN Versionsbump:** der ausgelieferte App-Build ist unverändert, eine Phantom-Release ohne sichtbare Änderung würde nur die Release-Notes verwässern. |
| v1.9.0 | ⏳ Geplant | Monster-Tracker (Live-HP) – verschoben von 1.4.0. Der Platz dafür ist im Session-Überblick bereits reserviert (fünfter Abschnitt „Am Tisch", nur während eines laufenden Szenarios). |
| v1.10.0 | ⏳ Geplant | Helden-Spieleransicht |
...
└── src/
    ...
    ├── utils/rumorTiming.ts ← Gerüchte: Zeitfenster-Parser + rumorPlayableNow (rein, v1.8.0)
    ├── components/
    │   ├── MapBuilder/     ← MapGrid, TileSidebar, constants
    │   ├── session/        ← Redesign v1.8.0: SetupSection, HeroesSection, HeroSheet,
    │   │                     OverlordSection, OverlordCards, HistorySection,
    │   │                     AdventureCardsSection, overview/, flow/ (5 Schritte + Draft),
    │   │                     ui/ (CardTile, CardThumb, HeroChipRow, Timeline,
    │   │                     NextScenarioCard, controls), scenarioSuggest, ItemPicker,
    │   │                     ui.tsx (ItemThumb/QtyStepper), sessionHelpers
    │   └── ...
    └── pages/
        ├── HomePage.tsx
        ├── MapBuilderPage.tsx
...
- **Nach JEDEM Merge nach `main` den GitHub-Pages-Build prüfen (User-Vorgabe 2026-07-31)** —
  und zwar **beide** Workflows: `Deploy to GitHub Pages` (eigener) **und**
  `pages-build-deployment` (GitHubs eingebauter). Ein grüner eigener Deploy-Workflow ist
  KEIN Beweis, dass Pages sauber gebaut hat. Hintergrund + gelöster Wettlauf-Konflikt:
  `wiki/concepts/dependency-security-pass.md` (Abschnitt „Nachtrag")
...
|---|---|
| React 19 + Vite 8 | Frontend-Framework + Build (Vite 8 = Rolldown; Node ≥ 22 erforderlich) |
| TypeScript | Typsicherheit |
| Tailwind CSS | Styling (ACHTUNG: `max-width: none` auf Tile-Images nötig!) |
| zustand + persist | State Management + localStorage |
| react-router (v8) | Routing (BrowserRouter mit `basename` + 404.html-SPA-Fallback für GitHub Pages). **`react-router-dom` gibt es ab v8 nicht mehr** — immer aus `react-router` importieren |
