...
| v1.6.8 | 🔄 In Arbeit | **Kartentext-Treue-Pass + Errata-Overwrite-Korrektur.** User-Frage: Wurden deutsche Original-Kartentexte durch Errata-Fassungen im Haupttext überschrieben? **Antwort: Ja, vereinzelt** — der erste Audit-Ansatz war FEHLERHAFT und ist korrigiert. **Fehler im 1. Ansatz:** verglich die Daten gegen die **deutschen Repo-Scans** (`public/cards/de/**`) und schloss „keine Overwrites". Falsch, weil einige Scans **Errata-korrigierte Nachdrucke** sind — vom User bewiesen an **Schattendrache**: `shadow-dragon-act1-back.webp` zeigt bereits die Errata-Fassung („Sobald ein Held, der benachbart …"), NICHT den Erstdruck („Wenn ein Held dieses Monster von einem Nachbarfeld aus angreift …"). Daten==Scan==Errata → der Bildvergleich konnte den Overwrite prinzipiell nicht sehen. **2. Ansatz (korrekt, bildunabhängig):** Datentext direkt gegen die CRRG-Errata-„sollte lauten"-Fassung verglichen (Scan über 54 Textkorrektur-Errata, 22 mit hoher Ähnlichkeit, je fein original-vs-errata diskriminiert). Ergebnis: **echte Overwrites** (Haupttext = Errata- statt Erstdruck-Fassung): **Schattendrache/Schatten (korrigiert, User-Originaltext), Goblin-Schamane/Verzaubern, Grisban/Heldenfähigkeit, Teuflisches Ritual, Vielseitigkeit, Geomant/Erdbeben** (+ Geistsprecher: nur Herz/Herzen, trivial). Die übrigen 16/22 halten den **Erstdruck** (Errata NICHT eingearbeitet, z. B. Ritter ohne „leeren", Waldläufer „vor UND nach", Blutdurst „besiegt", Berserker behält den von der Errata gestrichenen Satz) → korrekt, kein Overwrite. **OFFEN (User-Quelle nötig):** für die 5 restlichen Overwrites fehlt der **Erstdruck-Originaltext** — Repo-Scans sind errata'd, kein zuverlässiger DE-Erstdruck im Repo, keine Rate-Übersetzung erlaubt. **Zusätzlich behoben — 18 errata-UNabhängige, gegen die Kartenbilder verifizierte Abschreibfehler** (Kartentext = priorisierte Wahrheit): 3 Overlord-Fallen „Wahrnehmung"→**„Gespür"**, Unterweltdrache+Dunkler Priester „Schreckenerregend/wendet -1 an"→**„Grauenerregend/verringert um 1"**, Plotkarte Bezaubern `[Geistesgegenwart]`→**`[Willenskraft]`**, Schattendrache/Feuerodem „(inklusive Zielfeld)" entfernt (Halbdrachenkrieger behält es), 5 Helden-Heldentaten (Lindel/Raythen/Roganna/Okaluk/Stahlhorn), Untoter Schädel, Harpyie/Pulk, Höllenkoloss/Sturmangriff, Windarmbänder/„sich", Zeichen des letzten Zenits/„sofort", Beastman/Kommando „darf"→„kann". Errata bleibt durchgehend additiv (ErrataBox). Docs synchron. 122 Tests + Build grün. **LEHRE (verbindlich):** Deutsche Scans können Errata-Nachdrucke sein → für die Errata-Overwrite-Prüfung Daten IMMER gegen die Errata-„sollte lauten"-Fassung vergleichen, NICHT gegen die Repo-Scans. |
